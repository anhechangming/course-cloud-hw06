...
public interface CatalogClient {
     ...
     * 1. 参数类型：String id（原Long错误）
     * 2. 返回值：Map<String, Object>（原CourseDto错误）
     * 3. 接口路径：/api/courses/{id}（匹配Controller的/{id}，前缀/api/courses由Controller类注解补充）
     */
    @GetMapping("/api/courses/{id}") // Controller类@RequestMapping("/api/courses") + 方法@GetMapping("/{id}") = 完整路径/api/courses/{id}
    Map<String, Object> getCourse(@PathVariable("id") String id); // 参数改为String
}